...
const 正しい状態 = {
    ロービーム: [lightSwitch.lowBeam, headlightLever.lowBeam, hazardLight.オフ],
    ハイビーム: [lightSwitch.lowBeam, headlightLever.highBeam, hazardLight.オフ],
    ハイビームとロービームの切り替え: [lightSwitch.lowBeam, headlightLever.highLowBeamToggle, hazardLight.オフ],
    ハザードランプ: [lightSwitch.positionLight, headlightLever.lowBeam, hazardLight.オン],
}
const selectedStatus = [lightSwitch.closed, headlightLever.lowBeam, hazardLight.オフ]
const sounds = [
...
document.getElementById("ハザードランプ").addEventListener('click', () => {
    if (selectedStatus[2] == hazardLight.オン) {
        selectedStatus[2] = hazardLight.オフ;
    } else if (selectedStatus[2] == hazardLight.オフ) {
        selectedStatus[2] = hazardLight.オン;
    } else {
